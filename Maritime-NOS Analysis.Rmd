...
ObservationCount = Total Count including the silent listening period.

**Clean sample data file included for ObservationCount1 (silent listening period)**
...
Code to read and append the relevant files from the out directory. This is ObservationCount1, which will be used for the national trends. 

```{r pull multiple files, echo=FALSE, message=FALSE}

#Read file that was created in ArcGIS by spatailly joining the Events layer (created in the data manipulation code) with the grid cells spatil layer. Need to manual rename columns before importing into R
...
out.dir<-"output/2019"
my.files<-list.files(path = out.dir, pattern ="*ObservationCount1.csv", full.name=TRUE)
obs<-ldply(my.files, read.csv)

my.files<-lapply(Sys.glob("*ObservationCount1.csv"), read.csv)


my.files<-list.files(path = out.dir, pattern ="*EventData.csv", full.name=TRUE)
...
Create summary stats for data checking
...
#species by protocol summary. Check for miss-classifications
...
#Get grid data for plotting, but first change the directory
  
ATOWLS_Grid <- readOGR(dsn="C:/Users/dethier/Documents/ethier-scripts/Maritime-NOS/data", layer="ATGrid_50Kv4")
